function [clusters,cuts,cheegers,vmin,fmin,normGrad] = ...
    pSpectralClustering(W,p,normalized,k)
...
%
%     figure(1);
%     gplotmap(W,coords,clusters(:,1))

for l=3:k
    bestCut     = inf;
    bestCheeger = inf;
    fprintf('Inside forloop for l=%d\n',l);
    for m=1:l-1
        ...
        if (~isempty(subClusters{m}))
            ...
            if (connected)
                ...
                if threshold_type>=0
                    ...
                    cutPart1          = computeCutValue_pspec(clusterM1,W,normalized); % vmin< threshold
                    
                    clusterM2         = zeros(size(allClusters,1),1);
                    clusterM2(indexM) = (allClustersInClusterM==0);
                    cutPart2          = computeCutValue_pspec(clusterM2,W,normalized); % vmin>threshold
                    ...
                    % calculate cuts with a sweep cut
                    degM = deg(indexM);
        ...
        %
        %             figure;
        %             gplotmap(W,coords,clusters_new)
    end
...
fprintf('Calls, Func: %d, Grad: %d, Hess: %d \n', func_counter, grad_counter, hess_counter);

fprintf('Counters, Newton: %d, SD: %d \n', Newton_counter, SD_counter);
